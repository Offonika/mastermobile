...
- `GET /api/v1/system/ping` — пинг сервиса, в ответе приходит timestamp, статус и результаты проверок зависимостей.
- `GET /api/v1/returns` — постраничный список возвратов (`page`, `page_size`).
- `POST /api/v1/returns` — создание возврата.
- `GET /api/v1/returns/{returnId}` — просмотр возврата по идентификатору (UUID).
- `PUT /api/v1/returns/{returnId}` — обновление возврата.
- `DELETE /api/v1/returns/{returnId}` — удаление возврата.

> Для модифицирующих операций (`POST/PUT/DELETE`) требуется заголовок `Idempotency-Key` (уникальное значение до 128 символов). Любой запрос может передать `X-Request-Id`; если он не задан, сервис сгенерирует идентификатор и вернёт его в ответе.

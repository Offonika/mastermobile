...
Проект: Курьерская доставка с «Ходячим складом»
 Версия: v0.3.2 (синхронизировано с 00‑Core v1.3)
 Дата: 19.09.2025
...
Оффлайн‑черновики: кэш рюкзака и черновиков → авто‑синк при сети с учётом ограничений платформы (на iOS отправка возможна только в форграунде/при повторном визите).
...
Запросы к API v1 передают заголовок X-Api-Version: 1.1.3 (актуальная минорная версия контракта). Ответы middleware возвращают заголовок API-Version: 1.1.3.
 ...
 События оффлайн-очереди повторной отправки (виджет → MW) инициируются только при активной сессии пользователя (foreground); на iOS фоновые Periodic Background Sync недоступны — пользователь получает напоминание об открытии виджета.
...
UAT‑07: Instant‑order → черновик 1С ≤3 мин; сумма совпадает; оффлайн‑черновик отправился при появлении сети и активной сессии. Для iOS проверяем, что очередь отправляется только в форграунде и делает до 3 ретраев с backoff; напоминание об открытии виджета доступно. (Согласовано с владельцами продукта Walking Warehouse — 19.09.2025.)
...
### Changelog
- v0.3.2 — non-breaking: уточнены ограничения авто‑синхронизации оффлайн‑черновиков (iOS — только форграунд), обновлены UAT для оффлайн‑сценариев; изменения согласованы с владельцами продукта Walking Warehouse (19.09.2025).

...
Проект: Курьерская доставка с «Ходячим складом»
 ...
 Связано: PRD «Ходячий склад» v1.3.2, ONE‑PAGER v1.3, Core Sync (ERP↔MW), API‑Contracts v1.0.0
...
1. Обзор и границы
Цель. Запустить управляемую доставку 1С ↔ Middleware ↔ Bitrix24 с «рюкзаком курьера», быстрой продажей «с места», фото‑контролем, фиксацией гео и корректным учётом наличных. Без создания новых типов документов в 1С (возвраты — только стандартный «Возврат товаров от покупателя» через мастер). Контуры интеграции не зависят от конкретной редакции ERP — обмен ведётся через согласованные API и справочники.
 In v1:
...
Чтобы снять назначение, передаём `{"courier_id": null}`; `{"decline": true}` фиксирует отказ курьера. Ответ 200 возвращает актуальный заказ; повтор с тем же Idempotency-Key идемпотентен.

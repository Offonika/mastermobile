...
Сущности (инициализация MW): returns, return_lines, integration_log, task_event; денежные поля при необходимости сопровождаются `currency_code` (ISO‑4217).


Бизнес‑правила:


Идемпотентность по Idempotency‑Key: ключ передаётся в запросах и сохраняется в `integration_log.correlation_id`; для событий Bitrix24 действует уникальность `task_event.correlation_id`.
...
6. API‑контракты (anchors v1.0.0)
 Общие заголовки (см. 00‑Core §2; API‑Contracts):
 Idempotency-Key: <uuid> (для POST/PUT/DELETE `/api/v1/returns`), X-Request-Id: <uuid> (опционально, MW генерирует при отсутствии), Authorization: Bearer <JWT>.
...
Примечание: POST /tasks/{id}/status удалён из актуального контракта (см. API‑Contracts).
...
Ошибки и форматы — см. Error Code Registry и API‑Contracts (application/problem+json, пагинация `page`/`page_size`).
...
Идемпотентность Idempotency‑Key фиксируется через `integration_log.correlation_id`; повтор с тем же ключом возвращает исходный ответ.
...
Логи JSON с X-Request-Id, маскирование ПДн.


integration_log ≥ 30 дн; task_event ≥ 180 дн.
...
Retention (см. 00‑Core §9): сырые фото (≤15 МБ) ≤90 дн; гео ≤30 дн; записи в `integration_log` (включая Idempotency-Key) ≥30 дн.
...
Сущности и поля — см. ER snapshot (инициализация MW): returns, return_lines, integration_log, task_event.
...
UAT‑RET‑01: гео удаляется ≤30 дн; повторный POST/PUT с тем же Idempotency-Key возвращает исходный ответ (подтверждено по integration_log.correlation_id).
...
Все UAT‑кейсы пройдены; повторные POST с тем же Idempotency-Key не создают дублей; фото/гео консистентны; ПКО‑черновик корректен; рюкзак списывается/пополняется без расхождений (инвентаризация ≥98%); логи/аудит полные; ПДн — под защитой; совместимость API подтверждена по актуальному контракту (`openapi.yaml`).

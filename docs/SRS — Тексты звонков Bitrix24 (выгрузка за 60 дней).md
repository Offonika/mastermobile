...
- FR-IDEMP-001. Повторный запуск с тем же периодом не создаёт дублей: существующие записи обновляются только если `status != completed` или отсутствует файл; уникальный ключ `call_records (run_id, call_id, coalesce(record_id,''))` предотвращает повторное добавление строк в реестре.
- FR-IDEMP-010. Запрос `/tasks/call_export` требует заголовок `Idempotency-Key` (UUID по 00‑Core §2.1); сервис сохраняет отпечаток тела запроса и при повторном вызове с тем же ключом возвращает исходный ответ без создания нового run (TTL хранения ключа — 72 часа).

...
FR‑IDEMP‑001. Идемпотентность. Повтор с тем же Idempotency‑Key возвращает исходный ответ; ключ сохраняется в `integration_log.correlation_id` и сравнивается при повторе.
...
Обязательные заголовки:
 Authorization: Bearer <JWT> • Idempotency-Key: <uuid> (для POST/PUT/DELETE `/api/v1/returns`) • X-Request-Id: <uuid> (опционально; MW генерирует при отсутствии).

На дату 2024-05-09 публичный контракт включает `/health`, `/api/v1/system/ping`, `/api/v1/returns` и `/api/v1/returns/{returnId}` (см. `openapi.yaml`).
 Кеш‑валидация: ETag/If-None-Match для GET/HEAD; weak‑ETag для списков.
 Идемпотентность PUT/DELETE: модифицирующие вызовы требуют Idempotency-Key; повтор с тем же ключом возвращает исходный ответ.
НСИ (примеры):
...
Производительность: p95 для MW ≤ 400 мс (без внешних вызовов); целевой throughput до 50 rps, burst 100 rps (≤60 c).
 ...
 Retention: integration_log ≥ 30 дн; task_event ≥ 180 дн; Idempotency-Key хранится через `integration_log.correlation_id`.
...
Автоматический деплой MW (blue‑green/rolling) с health‑чеками и авто‑откатом при деградации SLO.
 Версионирование API: префикс `/api/v1`, совместимость схем при минорных релизах.

...
Версия: 1.0.3 (синхронизировано с 00‑Core v1.3.3)
 Дата: 27.09.2025
 Владелец: Операции / IT
Связанные документы: PRD Core Sync v1.1.2 (18.09.2025), ONE‑PAGER (финал), API‑Contracts v1.1.1, ER Freeze v0.6.5
...
Контракт v1.1.1 опубликован в `openapi.yaml` и покрывает три домена: `returns`, `b24-calls`, `walking-warehouse`, а также системные проверки. Контракт синхронизирован с FastAPI-службой; любые изменения требуют обновления OpenAPI и SRS.
...
- POST `/api/v1/returns/{return_id}/reject` — отклонение возврата после проверки качества; принимает `ReturnRejectRequest` (reason_code `no_defect_found|other`, строки `items[]`, актор) и возвращает актуальный `Return` со статусом `rejected`. Доступно ролям `1c`, `admin`, требует `Idempotency-Key`.

`b24-calls` (выгрузка звонков Bitrix24):
- GET `/api/v1/b24-calls/export.csv` — потоковая выгрузка в UTF‑8 CSV (фильтры: сотрудник, диапазон дат, наличие текста).
- GET `/api/v1/b24-calls/export.json` — та же выборка в JSON (массив `B24CallRecord`).
Обе операции возвращают 400 при некорректном диапазоне дат и 422 при ошибках валидации.
...
## Changelog
- 25.09.2025 — Перешли на API-Contracts v1.1.0: добавлены домены `b24-calls` и `walking-warehouse`, расширены требования к API.

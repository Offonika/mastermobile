...
Поле‑в‑поле (таблица в приложении): единицы, НДС, валюты, статусы, округления (цены/суммы — numeric(12,2)), часовые пояса (все метки — UTC).
 ...
 Актуальный срез ER Freeze v0.6.4:
  - `core` — orders, order_lines, returns, return_lines, return_reason, task_events, integration_log, idempotency_key, status_dict.
  - `ww` — couriers, courier_stock, instant_orders, instant_order_lines, price_type, product (заготовка под ассортимент).
  - `catalog.*`, `logistics.*`, `sales.*`, `finance.*` отсутствуют; эти схемы остаются на стороне 1С до релиза ER v0.7. Соответствующие требования в этом SRS помечены как плановые.
...
## Changelog
- 01.10.2025 — Уточнена связь с ER Freeze v0.6.4: перечислены фактические таблицы (`core.*`, `ww.*`) и помечены плановые схемы (`catalog.*`, `finance.*`, `logistics.*`, `sales.*`) для согласования с интеграционным маппингом.

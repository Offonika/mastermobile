...
| Запросы из непроверенных доменов | Браузерные клиенты с чужих origin | Строгий CORS allow-list для публичного API (переменная `ALLOWED_CORS_ORIGINS`), принудительно применяется на уровне API Gateway/FastAPI middleware; мониторинг конфигурации |

...
Статус: финал для v1 (единый для обоих проектов)
 Основание: выровнено с 00‑Core v1.3; PRD «Ходячий склад» v1.3.1; SRS «Рюкзак» v0.3; PRD Core Sync v1.1.1; SRS Core Sync v1.0.1; ER Freeze v0.6.1
...
8) Примеры cURL
(см. версию v1.1.1 — без изменений, дополнены заголовки X-Api-Version и Idempotency-Key).
...
Таблица ролей/эндпоинтов совпадает с v1.1.1. Дополнено ограничение "только свои" по courier_id/владению задачей (проверка на MW) и обязательные scopes (nsi.read|write, orders.write, returns.write, tasks.read, webhooks.sign).

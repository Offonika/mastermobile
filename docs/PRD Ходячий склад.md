...
Runbook: [операционные логи Walking Warehouse](runbooks/ww.md) — порядок доступа и расследования инцидентов.
 Дата: 18.09.2025
 Цель: Запустить прозрачный процесс курьерской доставки и возвратов поверх MasterMobile Middleware без смены учетной платформы, с «рюкзаком курьера» для допродаж.

1. Зачем (коротко)
Проблема. В текущей учетной системе нет стабильного контура доставки; обмен с сайтом нестабилен.
...
Scope v1 (что делаем):
Учетная система: кнопка «Отправить в B24 (курьер)», авто‑статус «Готов (B24)», нужные поля заказа, хранение TaskIdB24 и IdempotencyKey.

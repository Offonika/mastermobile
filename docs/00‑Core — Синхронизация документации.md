00‑Core — Синхронизация документации (v1.0)
...
Ретраи на стороне клиента разрешены при 5xx, timeout, integration.* с экспоненциальной паузой (до 24 ч).
...
Circuit breaker для интеграций: порог отказов 50% за 60 сек → полууоткрытое состояние 30 сек.
...
3.3. Возвраты (return / return_line)
 Подпроцесс: return_ready → accepted | rejected.
...
4. ER Freeze v0.6.1 (единая БД MW)
Ключевые сущности:
...
SRS «Рюкзак»: заменить X-Request-Id на Idempotency-Key; добавить X-Correlation-Id; сослаться на Status‑Dictionary v1, ER Freeze v0.6.1 и разделы Environments/DR/Retention; удалить локальные дубли NFR; описать webhook signature и rate limits.
...
ER/DDL репозиторий: выпустить тег schema‑v0.6.1‑freeze; миграции DEV; добавить idempotency_key TTL‑cleanup job; описать expand‑migrate‑contract стратегию.
...
ER Freeze v0.6.1 и миграции/rollback‑plan (4, 6.3)
...
/v1/returns — заявки на возврат, решения accepted/rejected.
...
«Проведён» ↔ accepted; «Отклонён» ↔ rejected
Номенклатура
nsi_item
GUID

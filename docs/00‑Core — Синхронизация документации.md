...
00‑Core — Синхронизация документации (v1.3.7)
Дата обновления: 03.10.2025
Связанные артефакты: API‑Contracts v1.1.0, SRS Core Sync v1.0.3, [ADR-0002](adr/0002-latency-slo.md)

> Обновление 03.10.2025. Подтверждена SoT-матрица владельцами доменов (1С Lead, Core Sync, Walking Warehouse): возвраты фиксируются в 1С как SoT, middleware остаётся источником оперативных статусов доставок, задач и мгновенных продаж; таблица дополнена колонкой владельцев.
...
| Ассистент мастера | [PRD — «Ассистент мастера» v0.1.1 (Final)](PRD%20—%20«Ассистент%20мастера».md) | — | 00‑Core v1.3.7, [API‑Contracts v1.0.0](API%E2%80%91Contracts.md), [Status‑Dictionary v1](00-Core_glossary-status.md#status-dictionary-v1), [SoT = Bitrix (витрина) / MW (оперативные статусы)](00%E2%80%91Core%20%E2%80%94%20%D0%A1%D0%B8%D0%BD%D1%85%D1%80%D0%BE%D0%BD%D0%B8%D0%B7%D0%B0%D1%86%D0%B8%D0%B8.md#5-sot-matrix-v1-source-of-truth) |
| КМП4 | — | [SRS — КМП4 (в подготовке)](SRS%20—%20КМП4.md) | Исходные материалы: `1c/config_dump_txt/`, `1c/external/kmp4_delivery_report/` (исходники в `src/`); привязка к 00‑Core v1.3.7 |
...
2.7. NFR/SLA
Цели p95: чтение ≤ 250 мс, модифицирующие операции ≤ 400 мс (см. [ADR-0002](adr/0002-latency-slo.md)).
...
Полный словарь статусов и глоссарий вынесены в [docs/00-Core_glossary-status.md](00-Core_glossary-status.md#status-dictionary-v1). См. отдельный документ для детальной матрицы переходов, терминов (SoT, Delta/Зеркало, MW) и модели данных справочника `status_dict`.
...
> Протокол согласования 03.10.2025 (1С Lead, Core Sync Lead, Walking Warehouse PM): 1С сохраняет SoT по НСИ, финансовым документам и финальным решениям по возвратам; middleware отвечает за оперативные статусы доставок, задачи и мгновенные продажи.

| Объект | Source of Truth | Репликация/обмен | Владельцы |
| --- | --- | --- | --- |
| НСИ (товары, SKU, цены, совместимость) | 1С | Пуш/пул через Core Sync → MW (дельты + ночное зеркало) | 1С Lead (SoT), Core Sync Team (зеркало) |
| Заказы на доставку | 1С (документ), MW (оперативные статусы) | Создание в 1С → события статусов в MW → рассылка в Bitrix24/1С | Product 1С (SoT), Core Sync Lead (статусы) |
| Задачи курьера / события | MW | Вебхуки MW → Bitrix24 и 1С; обратная связь из Bitrix24 через API | Walking Warehouse PM, Core Sync Lead |
| Мгновенные продажи (рюкзак) | MW | Оперативный учёт в MW → проведение в 1С по событию accepted | Walking Warehouse PM (SoT), 1С Lead (финал) |
| Возвраты | 1С (финальное решение), MW (инициация) | Заявка создаётся в MW → подтверждение/проведение в 1С → обратная синхронизация статуса | 1С Lead (SoT), Core Sync Lead |
| Денежные операции (касса/ДВП) | 1С | Финансовые итоги в 1С → контрольные сверки (read-only) в MW | Финансы 1С (SoT), Core Sync Team |
...
SRS «Рюкзак»: заменить X-Request-Id на Idempotency-Key; добавить X-Correlation-Id; сослаться на [Status‑Dictionary v1](00-Core_glossary-status.md#status-dictionary-v1), ER Freeze v0.6.5 и разделы Environments/DR/Retention; удалить локальные дубли NFR; описать webhook signature и rate limits.
...
SRS Core Sync: ссылка на SoT‑Matrix v1, [Status‑Dictionary v1](00-Core_glossary-status.md#status-dictionary-v1), DR/BCP; синхронизировать коды ошибок по Core‑API‑Style; добавить TTL идемпотентности.
...
Правила:
 ...
 — Статус‑маппинги должны ссылаться на [Status‑Dictionary v1](00-Core_glossary-status.md#status-dictionary-v1).

...
- Точность извлечения сущностей (golden-set `docs/NLU/golden_set.csv`) ≥ 80% F1.
...
2. **Неделя 1 — Поиск и NLU**
   ...
   - Подготовить golden set (`docs/NLU/golden_set.csv`), автотесты для поиска и базовые KPI (CTR, конверсия, F1), чтобы отслеживать прогресс пилота.

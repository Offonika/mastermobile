...
> Примечание (26.09.2025). Аудит артефактов подтвердил, что актуальная целевая версия API‑Contracts — v1.1.0. Ссылки на v1.1.3
...
- Схема безопасности — `bearerAuth` (`JWT`). Токен обязан содержать роль, разрешённую для конкретной операции:
  ...
  - `walking-warehouse`: см. раздел 4 (роли `courier`, `admin`).
- Системные проверки (`/health`, `/api/v1/system/ping`) анонимны и не предъявляют требований к заголовку `Authorization`.
...
### 1.5 Пагинация
- Все коллекции с пагинацией принимают `page` (≥ 1, по умолчанию 1) и `page_size` (1..100, по умолчанию 20).
- Ответы используют схемы `Paginated*` с полями `items[]`, `page`, `page_size`, `total_items`, `total_pages`, `has_next`.
- Исторические параметры `limit`/`offset` больше не поддерживаются и должны быть исключены из интеграций.
...
- Заголовки: `Authorization: Bearer <JWT>` (обязательный), опционально `X-Request-Id`.
...
### 4.3 GET /api/v1/ww/orders
- Роли: `courier`, `admin`.
- Параметры: `status[]`, `q`.
- Ответ 200: `OrderListResponse`.
- Ошибки: 401, 403, 422.

### 4.4 POST /api/v1/ww/orders
- Роли: `courier`, `admin`.
- Заголовки: `Authorization`, `Idempotency-Key`, `X-Request-Id` (опц.).
- Тело: `OrderCreate`.

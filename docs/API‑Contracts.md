...
## 3. Матрица статусов доставки
Статусы `delivery_order` и связанные задачи `task` синхронизируются между 1С и мобильным приложением. Матрица фиксирует допустимые переходы и ответственных акторов. Любые изменения требуют обновления `status_dict`, 00‑Core (§3) и автоматических тестов.

| Текущий статус | Допустимые следующие | Кто инициирует | Примечания |
| --- | --- | --- | --- |
| `NEW` | `PICKING` | MW (авто) | Создание заказа на складе. |
| `PICKING` | `READY` | MW / склад | Комплектация завершена, заказ ожидает курьера. |
| `READY` | `PICKED_UP`, `REFUSED` | MW (скан) / менеджер | Отказ (`REFUSED`) фиксируется менеджером, заказ возвращается в 1С. |
| `PICKED_UP` | `ON_ROUTE` | Курьер | Курьер покидает склад с заказом. |
| `ON_ROUTE` | `DELIVERED`, `FAILED` | Курьер | `FAILED` используется при нарушении SLA, требует ручной обработки. |
| `DELIVERED` | `CASH_RETURNED`, `FAILED` | Курьер | При безналичной оплате переход на `CASH_RETURNED` пропускается. |
| `CASH_RETURNED` | `DONE` | Курьер / касса | Возврат наличности подтверждён. |
| `DONE` | — | MW | Финальный статус. |

Дополнительные правила:
- `FAILED` доступен только из `ON_ROUTE` и `DELIVERED`; перевод инициирует курьер с обязательным комментарием.
- `REFUSED` доступен только из `READY` и выставляется менеджером или MW по интеграции 1С.
- Статусы `CANCELLED`, `NON_CASH_CONFIRMED`, `CLOSED` помечены как deprecated и не используются новыми интеграциями.

## 4. Эндпоинты блока returns
### 4.1 GET /api/v1/returns
...
### 4.5 DELETE /api/v1/returns/{returnId}
